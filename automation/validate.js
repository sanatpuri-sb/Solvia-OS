...
function referentialIntegrity() {
    ...
    for (const t of tasks) {
        ...
        const badBeliefs = (t.links?.belief_ids || []).filter(id => !beliefIds.has(id));
        if (badGoals.length) {
            console.error(`❌ Task ${t.id} links unknown goal IDs: ${badGoals.join(', ')}`);
            ok = false;
        }
        if (badBeliefs.length) {
            console.error(`❌ Task ${t.id} links unknown belief IDs: ${badBeliefs.join(', ')}`);
            ok = false;
        }
    }
