...
> Goal: Minimize manual window. In ~1–2 days, bring up **persistent memory**, **MCP runtime**,
...
> linear history**;  
> “Require approvals” and “Require status checks” are **temporarily disabled** to allow solo
> merges.  
...
- [x] Verify VS Code validation works (introduce a fake key in `memory/tasks.yaml` → red squiggle
      appears)
- [x] Mark invalid PRs as **fail** (no merge)
- [x] Main branch must be protected in GitHub settings; require CI pass before merge
- [x] Branch protection → Required status checks: add **`validate`** (use the exact name shown in
      your PR Checks)
...
_(If you don’t have scripts yet, stub the job to at least lint YAML + deny unknown paths; you can
harden later.)_
...
- [ ] Add a CI job to auto-format Markdown (Prettier) before enforcing strict markdownlint ⟵ (keep
...
- [ ] Manually trigger Proposals Cron → a PR appears with a trivial, valid change (e.g., a new tag
      in `identity.yaml`)
...
- [ ] (Optional) Expose a small HTTP tool for **retrieve_context** → returns a context pack from
      vector store

**Go/No-Go F**

- [ ] In Claude: “Open `identity.yaml` → prepare a proposal to add a minor key (e.g.,
      `docs.master_plan=/docs/master_plan.md`)”
...
- MCP runtime must never be allowed to bypass proposal workflow — disable direct main-branch writes
